...
- Location-aware site footprint: solar land from panel area ÷ GCR (latitude-adjusted row spacing, NREL/SAM-style), plus **cited or screening** process-pad intensities (PEM, DAC, Sabatier, SWRO/MED, brine, ASU, Haber–Bosch, battery). The Footprint map layer draws separate solar + pad polygons in a deterministic campus layout (not one total circle). Network `landHa` sums those site totals, not a flat 1.6 ha/MWp.
