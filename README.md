...
- A location bar: a Leaflet map picker plus coordinates. Click the map or Apply location to fetch or reuse PVGIS typical-day solar and bind it to the current factory. Toggle OSM, a GSA-like solar GHI colormap (`data/ghi-coarse.json`, screening field, not the GSA raster; hourly PVGIS still fetched on Apply location), WRI Aqueduct Baseline Water Stress tiles (UMN SCOPE MapServer, `{z}/{y}/{x}`, cited, with a screening GridLayer fallback), a pale-to-dark-green land-value choropleth (screening, not cadastral), `estimateFootprint` pads, and network plant markers. Solar, water, and land colormaps are mutually exclusive. If tiles or the map library fail, lat/lon still work. Site panels keep meteo cites, assay summaries, and rights chips (`authorized` / `assumed` / `unverified`) with a light `kind` (`grid` / `freshwater` / `intake` / `discharge` / `concession` / `purchase`). Unverified grid and freshwater stay explicit zeros until assigned (`authorize: false`); solve warns `unverified site right: KEY`. Size-to-target will not grow a resource whose right is unverified (`size-to-target cannot assume KEY`). Intake is not discharge; a concession is not a purchase.
...
- `engine/map-site.js`: map-picker geometry, layer cites, and colormap helpers (`ghiAt` / GSA-like ramp, Aqueduct BWS tile template, land-value choropleth). Not a permit, assay, cadastral layer, or surveyed layout. `data/ghi-coarse.json` is the bundled screening GHI field.
