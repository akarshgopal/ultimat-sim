...
const Diagram = {
  ...
  syncTheme() {
    ...
    this.colors = {
      ...
      water: read('--water', this.colors.water),
      brine: read('--brine', this.colors.brine),
      heat: read('--heat', this.colors.heat),
  ...
  buildConnections(nodes, r) {
    ...
    r.exploratoryModules.filter(module => module.enabled).forEach(module => {
      ...
      if (diagramInputs.brine) {
        const desalNode = get('exp-desalination');
        if (desalNode && node !== desalNode) {
          connections.push(this.conn(desalNode, node, this.colors.brine, this.formatLabeledUnitRate((r.waterSystems?.brineDailyM3 || 0) / Math.max(cycleHours, 1), 'm3', 'Avg'), true, {
            width: 1.6, route: 'vertical', fromOffsetX: branchOffset(desalNode, node, 24),
          }));
        }
      }
