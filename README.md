...
- Cited inland/coastal **process-brine** assays (Abu Dhabi sabkha, Salar de Atacama Li, Lake Mackay WA, Great Salt Lake) sit beside the Millero seawater files. Geography-matched coasts bind both: Gulf presets get sabkha brine, Mejillones gets Atacama nucleus brine, Port Hedland gets Lake Mackay playa brine as WA salt-lake regional screening — seawater resources are not overwritten. `createAbundanceCase({ assayId })` (default `dead-sea-brine`) can run the minerals hub on those feeds; missing bromide is zero, not invented. A literature assay is not a mineral concession.
- Location presets for real candidate coasts: Gulf (Taweelah, Ras Laffan), Texas/US Gulf (Corpus Christi), India (Mundra), Australia (Pilbara Port Hedland, Kwinana, inland Lake Mackay brine-hub), North Africa (Agadir, Dakhla), Atacama/Chile (Mejillones), plus Red Sea (NEOM Oxagon, Ain Sokhna), Oman Duqm, Namibia Walvis Bay, and Almería. Selecting a site sets coordinates, name, and honest unverified/assumed rights, then fetches live PVGIS. High-signal coasts bind a frozen Millero-scaled multi-ion seawater assay (Persian Gulf, Red Sea, Texas Gulf, Pilbara/Indian Ocean, Atacama Pacific, Morocco Atlantic, Almería) into `site.assay` / `site.resources.seawater`; Oman Duqm, Mundra, and Walvis Bay stay screening — no intake-quality published composition was pinned for those three, so nothing was invented. These are map locations only — they do not replace the Almería coastal methane, Mejillones methanol, or Dead Sea abundance demos, and they are not bankable permits.
