...
test('water loop makes permeate, brine, ion concentration, and electrolyzer water credit explicit', () => {
  const result = runScenario({ desalinationEnabled: true, electrolyzerEnabled: true });
  const water = result.waterSystems;

  assert.ok(water.annualM3 > 0, 'Desalination should produce fresh water.');
  assert.ok(water.brineAnnualM3 > 0, 'Desalination should produce a brine stream.');
  assert.ok(water.ionVector.Na > 10.8, 'RO reject should concentrate feed ions.');
  assert.equal(water.waterCreditDailyM3, water.freshWaterDailyM3, 'Permeate should credit electrolyzer process water before sale.');
...
test('brine mining is feed-limited and hard-gates lithium from seawater', () => {
  ...
  assert.ok(salts.waterSystems.mining.products.NaCl.annualTons > 0, 'Salt suite should produce NaCl from SWRO brine.');

  const lithium = runScenario({
