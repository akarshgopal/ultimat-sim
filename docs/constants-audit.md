...
| `units.js` / catalog | `secKWhPerM3: 3.5` (high-recovery preset 4.5) | SWRO electricity | **cited** | Elimelech & Phillip 2011 plant SEC 3–4 kWh/m³ (RO plus intake/pretreatment/posttreatment/brine). Ghaffour et al. 2013 same 3–4 kWh/m³ band with energy recovery. Voutchkov 2018 best-in-class RO-train 2.5–2.8 kWh/m³ (DOI 10.1016/j.desal.2017.10.033); 3.5 keeps plant-level band. |
...
| `cases/coastal.js` | Alboran MAW 36.5 g/kg multi-ion; intake 0.1 m³/day; density 1.026 kg/L | Feed composition / budget | **cited** (Millero-scale assay) + **assumed** (intake) + **unverified** (discharge) | Frozen `data/almeria-seawater.json` retrieved 2026-09-06. Millero/Pilson S=35 majors × 36.5/35. Not a NaCl proxy. `seawaterIntake` assumed; `seawaterDischarge` unverified (no outfall). |
| `data/*-seawater.json` + `site-assays.js` | Regional Millero-scale assays (Gulf 40.5, Red Sea 40.5, Texas 36.2, Pilbara 35.5, Atacama 34.9, Morocco 36.5, Arabian Sea 36.3, Gulf of Kutch 37.2, Benguela 35.2) | Preset site.assay / seawater resource | **cited** (composition) + **screening** (intake volume) | Frozen 2026-09-13 (original six) and 2026-09-21 (Duqm/Mundra/Walvis). Same Millero majors scaled by cited regional salinity. Registry binds on preset select. Duqm = Arabian Sea basin typical (Kumar & Prasad 1999; Rao & Sivakumar 2003). Mundra = Gulf of Kutch mouth 37.20 psu (Vethamony et al. 2007); inner gulf saltier. Walvis = northern Benguela / SE Atlantic ~35.0–35.5 (Mohrholz et al. 2008; WOA 2023). Basin typical ≠ intake permit. Coastal/methanol search skips sites without frozen PVGIS (`no-frozen-pvgis`) rather than inventing kWh/kWp. Per-site ERA5 PVcalc monthly series frozen 2026-09-21: Duqm, Mundra, Walvis Bay, Taweelah, Ras Laffan, Oxagon, Ras Al-Khair, Corpus Christi, Port Hedland, Agadir, Ain Sokhna, Yanbu, Kwinana, Dakhla, plus inland Lake Mackay / Great Salt Lake / Salar de Atacama (`data/pvgis-*.json`; not Almería or Mejillones kWh/kWp). Existing Almería (2026-09-05 SARAH3), Mejillones (2026-09-14 ERA5), and Dead Sea (2026-09-06 SARAH3) JSON are registered in `pvgis-sites.js` `BY_SITE_ID`. |
...
| location | symbol/value | used for | class | proposed source or action |
| --- | --- | --- | --- | --- |
| generic / `dac-solid` | capture 0.9; elec 0.5; heat 1.5 kWh/kg; minHeat 80 °C; makeup 0.02; waste 40 °C | Default solid-sorbent screening | **assumption** / **screening** | IEA DAC 2022 S-DAC family: 0.5 kWh/kg e = 1.8 GJ/t; 1.5 kWh/kg th = 5.4 GJ/t; 7.2 GJ/t at the low end of 7.2–9.5. NASEM 2019 (DOI 10.17226/25259) notes thermal regeneration dominates solid-sorbent DAC. Capture 0.9 and makeup 0.02 are screening, not IEA/NASEM table values. |
...
| abundance / Dead Sea brine assay | Wikipedia early-1980s surface ions (Cl 181.4, Mg 35.2, Na 32.5, … g/kg); Li⁺ 18 mg/L | Feed composition | **cited** | Frozen `data/dead-sea-brine.json` retrieved 2026-09-06. Majors from Wikipedia Dead Sea chemistry. Li from Alsabbagh et al. 2021 (DOI 10.1016/j.mineng.2021.107038) converted at 1.24 kg/L → 0.018/1.24 ≈ 0.0145 g/kg. Daily feed 1e5 kg. Literature assay is not a mineral concession. `brineConcession` unverified; seawater intake/discharge unverified inland; freshwater and salt purchase assumed. |
| process-brine assays (sabkha, Atacama, Mackay, GSL, Salton Sea, Uyuni, Qaidam, Danakil, Searles, Red Sea sabkha, Kutch, Texas desal-reject, Mediterranean SWRO) | Cited inland/coastal process-brine or desal-reject majors (not Millero seawater). Gulf sabkha Wood 2002 / MDPI Hydrology 2020; Atacama + GSL + Uyuni Murphy & Haji 2022 Table 1; Lake Mackay Agrimin EPA memo medians; Salton Sea Stringfellow & Dobson 2021 Energies Table 2 / NREL TEA OSTI 1782801 ppm family; Qaidam Kunteyi Ren et al. 2023 Table 1 sample 5; Danakil Dallol Garrett 2004 / Martini 1969; Searles Teeple 1929; Jizan Red Sea sabkha Basyoni & Mousa 2016 Table 2 means; Kutch Rebary & Patidar 2021 Table 6 medians; Corpus Christi PCCA design-basis Outfall 001; Villar et al. 2023 Mediterranean SWRO reject. Ions omitted when not in the cite (no invented Li+/Br−). | Preset `site.resources.brine` / `createAbundanceCase({ assayId })` | **cited** (composition) + **screening** (density, intake volume) | Frozen in `data/*-brine.json` (2026-09-20/21). Bound where geography holds (Gulf sabkha coasts; Mejillones→Atacama; Port Hedland/Kwinana→Mackay; Oxagon/Sokhna/Yanbu→Jizan; Mundra→Kutch; Corpus→desal-reject; Almería→SWRO; inland Uyuni/Qaidam/Danakil/Searles/GSL/Salton/Mackay/Atacama). Duqm/Agadir/Dakhla/Walvis permanent-skip (no cited mean). Literature ≠ concession. |
| site rights `kind` / `authorize` | `grid` `freshwater` `intake` `discharge` `concession` `purchase`; `authorize` true for authorized/assumed, false for unverified | Access vs presence | **n/a** | Intake is not discharge. A concession is not a purchase. Size throws `size-to-target cannot assume KEY` rather than growing an unauthorized resource. Grid and freshwater stay zero when `authorize` is false. |
...
| `data/tea-screening.js` `capexMultiplierByRegion` | 0.75–1.2 screening location factors vs US Gulf-ish 1.0 (India 0.75, Levant 0.85, Gulf 0.90, Southern Africa 0.95, Texas 1.0, Chile 1.05, Australia 1.15, Europe 1.20; `default` omitted → 1) | Regional CAPEX intensity via `bindCapexPack({ region })`: `capexIntensity_region = pack.capexIntensity × multiplier[region]` | **screening** | Labor/construction/EPC proxy from Turner & Townsend GCMI / World Bank ICP / IRENA TIC families as direction; construction spread damped into ~0.7–1.3 because process equipment is traded. Not plant quotes. NREL ATB is US-only. |
| `data/tea-screening.js` `solarCapexByRegion.india` | $525/kWp | solar-pv CAPEX overlay (replaces pack $1000 × India 0.75 for solar-pv only) | **screening** | IRENA Renewable Power Generation Costs in 2024 India utility-PV TIC ~$525/kW vs US ~$1,058/kW. Not stacked with the process-plant multiplier. Not a Mundra EPC quote. |
| `data/tea-screening.js` `priceByRegion['chile-atacama'].lithium` | $14/kg | Chile Li unit price overlay | **cited** | Same USGS MCS 2025 LCE proxy as the global table; Chile is supply-side, not a LiCl contract. |
| `data/tea-screening.js` `inherit:'me-levant'` on demand rows | flag + note | Non-ME fuel/chem (and un-overridden mineral) offtake caps copied from me-levant | **screening** | Explicit inherit — not a silent Dead Sea copy. Chile/AU/TX/… Li rows are regionalized and must not carry the flag. |
